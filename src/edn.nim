...
proc new_edn_string_move(s: string): EdnNode {.gcsafe.} =
...
proc new_edn_bool*(val: bool): EdnNode {.gcsafe.} =
  case val
...
proc new_edn_bool*(s: string): EdnNode {.gcsafe.} =
  let parsed: bool = parseBool(s)
  return new_edn_bool(parsed)

proc new_edn_symbol*(ns, name: string): EdnNode {.gcsafe.} =
...
EdnTrue = edn_true # maybe not needed because Nim's case and _ insensitivity
EdnFalse = edn_false # maybe not needed because Nim's case and _ insensitivity
...
proc parse_string(p: var EdnParser): TokenKind {.gcsafe.} =
  result = tkString
  var pos = p.bufpos
  var buf = p.buf
  while true:
...
proc read_string(p: var EdnParser): EdnNode {.gcsafe.} =
...
proc read_quoted_internal(p: var EdnParser, quote_name: string): EdnNode {.gcsafe.} =
  let quoted = read(p)
  result = EdnNode(kind: EdnList)
  result.list = @[new_edn_symbol("", quote_name), quoted]

proc read_quoted*(p: var EdnParser): EdnNode {.gcsafe.} =
  return read_quoted_internal(p, "quote")

proc read_quasi_quoted*(p: var EdnParser): EdnNode {.gcsafe.} =
  return read_quoted_internal(p, "quasiquote")

proc read_unquoted*(p: var EdnParser): EdnNode {.gcsafe.} =
  return read_quoted_internal(p, "unquote")

proc read_deref*(p: var EdnParser): EdnNode {.gcsafe.} =
  return read_quoted_internal(p, "deref")

# TODO: read comment as continuous blocks, not just lines
proc read_comment(p: var EdnParser): EdnNode {.gcsafe.} =
  var pos = p.bufpos
  var buf = p.buf
...
proc read_token(p: var EdnParser, lead_constituent: bool): string {.gcsafe.} =
  var pos = p.bufpos
  var ch = p.buf[pos]
...
proc read_character(p: var EdnParser): EdnNode {.gcsafe.} =
  var pos = p.bufpos
  #var buf = p.buf
  let ch = p.buf[pos]
  if ch == EndOfFile:
...
proc interpret_token(token: string): EdnNode {.gcsafe.} =
  result = nil
  case token
  of "nil":
    result = new_edn_nil()
